...
npm run lint           # eslint . (ESLint 9 flat config; `next lint` was removed in Next 16)
npm run typecheck      # tsc --noEmit
npm run test           # OFFLINE: validates agent output schemas + the zod→JSON-schema converter (no keys needed)
npm run test:unit      # OFFLINE: crypto, billing/plans, serialize (incl. XSS sanitize), chunking, buckets
npm run test:webhook   # OFFLINE: Stripe webhook mapStatus/resolvePlan logic
npm run test:e2e       # LIVE: runs the full pipeline twice (strong + weak signal); needs DB + an LLM key + migrations applied
npx prisma migrate dev --name <change>  # create + apply a migration locally (migrations are committed)
npx prisma migrate deploy               # apply committed migrations (prod/CI; uses DIRECT_URL)
npm run db:push        # THROWAWAY local experiments only — schema changes go through prisma migrate
...
npx tsx scripts/migrate-v5-pgvector.ts  # historical: the HNSW index is now in prisma/migrations/0_init; only needed after a bare db:push
